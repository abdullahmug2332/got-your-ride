...
export default function Card(props) {
    ...
    useEffect(() => {
        if (props.place == 1) {
            ...
            setPrice(props.people*380)

        } else if (props.place == 2) {
            setImg(img2)
            setPlacename("Mount Fuji")
            setPrice(props.people*408)

        } else if (props.place == 3) {
            setImg(img3)
            setPlacename("Nagano")
            setPrice(props.people*477)

        } else if (props.place == 4) {
            setImg(img4)
            setPlacename("Nikko")
            setPrice(props.people*423)
    ...
    return (
        <div onClick={handleClick} className='flex items-center px-[20px] pr-[5px] my-[15px]'>
            <div className='w-[33%] flex gap-[15px]'>
                ...
                <div className='flex flex-col justify-between'>
                    ...
                    <p className='text-[14px] text-[#f0582a] font-extrabold'>${price}</p>
                </div>
            </div>
